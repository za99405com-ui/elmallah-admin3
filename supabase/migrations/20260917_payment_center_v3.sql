...
-- Synchronize deposit_required with default_payment_policy if already set
...
BEGIN
    ...
    UPDATE public.payment_sessions
       SET device_id = v_device.id,
           ...
           payment_destination = COALESCE(v_source.destination, v_device.payment_destination),
           payment_source_id = COALESCE(v_source.id, payment_source_id),
           updated_at = now()
     WHERE id = p_session_id;

    RETURN QUERY SELECT v_device.id, v_device.device_id, COALESCE(v_source.destination, v_device.payment_destination);
END;
$$;
